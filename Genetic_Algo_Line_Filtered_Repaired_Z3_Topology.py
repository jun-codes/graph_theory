...
OUTPUT_DIR = Path(BASE) / "outputs" / "line_topology_z3"
...
def topology_z3_then_gradient_repair(G, *, grad_steps=25, grad_lr=0.45, grad_max_move=12.0):
    ...
    G = _tag_topology_stats(G, topology_stats)
    G = _tag_z3_stats(G, z3_stats)
    return G, topology_stats, z3_stats, grad_stats
...
def run_ga(population_size=50, generations=MAX_GEN,
    ...
    for _ in range(population_size):
        ...
        while len(interior) < MIN_INTERIOR_EDGES and attempts < 10:
            ...
            interior = [(u, v) for u, v, d in G.edges(data=True)
                        if d.get('fold_type') != 1]
            attempts += 1
        G, topology_stats, z3_stats, stats = topology_z3_then_gradient_repair(
    ...
    for gen in range(1, generations + 1):
        ...
        while len(new_pop) < population_size:
            ...
            child, topology_stats, z3_stats, repair_stats = topology_z3_then_gradient_repair(
        ...
        if gen % 10 == 0:
            ...
            for G in new_pop:
                repair_symmetry(G)
                repaired, topology_stats, z3_stats, repair_stats = topology_z3_then_gradient_repair(
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "line_topology_z3_ga_convergence.png", dpi=150)
    plt.close(fig)
    ...
    for i, G in enumerate(diverse6):
        ...
        visualise(G,
                  ...
                  ax=axes.flatten()[i])
    plt.suptitle("Top 6 Topology + Z3 Maekawa-Repaired Crease Patterns", fontsize=13)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "line_topology_z3_ga_top6.png", dpi=150)
    plt.close(fig)

    with (OUTPUT_DIR / "line_topology_z3_best_generated.pkl").open('wb') as f:
        pickle.dump(best_ever, f)
    with (OUTPUT_DIR / "line_topology_z3_diverse_top6.pkl").open('wb') as f:
        pickle.dump(diverse6, f)
    print("Saved line_topology_z3_best_generated.pkl + line_topology_z3_diverse_top6.pkl")
