...
OUTPUT_DIR = Path(BASE) / "outputs" / "line_repaired"
...
def run_ga(population_size=50, generations=MAX_GEN,
    ...
    for _ in range(population_size):
        ...
        while len(interior) < MIN_INTERIOR_EDGES and attempts < 10:
            ...
            interior = [(u, v) for u, v, d in G.edges(data=True)
                        if d.get('fold_type') != 1]
            attempts += 1
        G, stats = gradient_repair_kawasaki_symmetry(
            G, steps=20, lr=0.45, max_move=10.0)
        repair_attempts += 1
    ...
    for gen in range(1, generations + 1):
        ...
        while len(new_pop) < population_size:
            ...
            child, repair_stats = gradient_repair_kawasaki_symmetry(
                child, steps=25, lr=0.45, max_move=12.0)
        ...
        if gen % 10 == 0:
            ...
            for G in new_pop:
                repair_symmetry(G)
                repaired, repair_stats = gradient_repair_kawasaki_symmetry(
                    G, steps=20, lr=0.35, max_move=8.0)
                gen_repair_attempts += 1
                gen_repair_accepts += int(repair_stats.accepted)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "line_repaired_ga_convergence.png", dpi=150)
    plt.close(fig)
    ...
    for i, G in enumerate(diverse6):
        ...
        visualise(G,
                  ...
                  ax=axes.flatten()[i])
    plt.suptitle("Top 6 Line-Filtered + Gradient-Repaired Crease Patterns", fontsize=13)
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "line_repaired_ga_top6.png", dpi=150)
    plt.close(fig)

    with (OUTPUT_DIR / "line_repaired_best_generated.pkl").open('wb') as f:
        pickle.dump(best_ever, f)
    with (OUTPUT_DIR / "line_repaired_diverse_top6.pkl").open('wb') as f:
        pickle.dump(diverse6, f)
    print("Saved line_repaired_best_generated.pkl + line_repaired_diverse_top6.pkl")
