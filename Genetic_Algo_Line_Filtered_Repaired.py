...
def run_ga(population_size=50, generations=MAX_GEN,
    ...
    plt.tight_layout()
    plt.savefig(f"{BASE}\\line_repaired_ga_convergence.png", dpi=150)
    plt.close(fig)
    ...
    plt.tight_layout()
    plt.savefig(f"{BASE}\\line_repaired_ga_top6.png", dpi=150)
    plt.close(fig)

    with open(f"{BASE}\\line_repaired_best_generated.pkl", 'wb') as f:
        pickle.dump(best_ever, f)
    with open(f"{BASE}\\line_repaired_diverse_top6.pkl", 'wb') as f:
