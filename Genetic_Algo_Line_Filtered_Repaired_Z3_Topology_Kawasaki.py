...
OUTPUT_DIR = Path(BASE) / "outputs" / "line_kawasaki_z3"
...
def topology_z3_then_gradient_repair(
    ...
    *,
    ...
    G, topology_stats = repair_even_nonborder_topology(G)
    ...
    G, grad_stats = gradient_repair_kawasaki_symmetry(
        ...
        symmetry_weight=grad_symmetry_weight,
    )
    G = _tag_topology_stats(G, topology_stats)
    G = _tag_z3_stats(G, z3_stats)
    G = _tag_kawasaki_projection_stats(G, projection_stats)
...
def find_mirror_node(G, node, candidates=None, max_dist=MIRROR_NODE_TOL):
    ...
    dist = math.hypot(G.nodes[best]['x'] + x, G.nodes[best]['y'] - y)
...
def mutate(G, gen=1, max_gen=MAX_GEN):
    ...
    # ── move_node ─────────────────────────────────────────────────────────────
        ...
        if any_incident_crosses(G, node, tree2, segs2, meta2):
            ...
            # Mirror most geometry moves to keep left/right pair structure.
            if random.random() < MIRROR_MOVE_PROB:
                mx, my = -nx_, ny_
...
def repair_symmetry(G, tol=30.0):
    lim = BORDER - 20
    mutable = [n for n in G.nodes() if is_mutable(G, n)]
    right = [n for n in mutable if G.nodes[n]['x'] > 0]
    left = [n for n in mutable if G.nodes[n]['x'] < 0]
    used_left = set()

    for r in sorted(right, key=lambda n: abs(G.nodes[n]['x']), reverse=True):
        rx, ry = G.nodes[r]['x'], G.nodes[r]['y']
        ...
        lx, ly = G.nodes[best_l]['x'], G.nodes[best_l]['y']
        dist = math.hypot(lx + rx, ly - ry)
        if dist > SCALE * 0.65:
            continue

        used_left.add(best_l)
        if dist <= tol:
            continue

        old_r = (G.nodes[r]['x'], G.nodes[r]['y'])
        old_l = (G.nodes[best_l]['x'], G.nodes[best_l]['y'])
        x_abs = np.clip((abs(rx) + abs(lx)) * 0.5, 0.0, lim)
        y_mid = np.clip((ry + ly) * 0.5, -lim, lim)

        G.nodes[r]['x'] = x_abs
        G.nodes[r]['y'] = y_mid
        G.nodes[best_l]['x'] = -x_abs
        G.nodes[best_l]['y'] = y_mid

        if any_incident_crosses(G, r) or any_incident_crosses(G, best_l):
            G.nodes[r]['x'], G.nodes[r]['y'] = old_r
            G.nodes[best_l]['x'], G.nodes[best_l]['y'] = old_l
...
def run_ga(population_size=50, generations=MAX_GEN,
    ...
    for gen in range(1, generations + 1):
        ...
        if SYMMETRY_REPAIR_INTERVAL and gen % SYMMETRY_REPAIR_INTERVAL == 0:
            repaired_pop = []
            for G in new_pop:
                repair_symmetry(G)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "line_kawasaki_z3_ga_convergence.png", dpi=150)
    plt.close(fig)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "line_kawasaki_z3_ga_top6.png", dpi=150)
    plt.close(fig)
    ...
    plt.tight_layout()
    plt.savefig(OUTPUT_DIR / "line_kawasaki_z3_violation_top6.png", dpi=150)
    plt.close(fig)

    with (OUTPUT_DIR / "line_kawasaki_z3_best_generated.pkl").open('wb') as f:
        pickle.dump(best_ever, f)
    with (OUTPUT_DIR / "line_kawasaki_z3_diverse_top6.pkl").open('wb') as f:
        pickle.dump(diverse6, f)
    print("Saved line_kawasaki_z3_best_generated.pkl + line_kawasaki_z3_diverse_top6.pkl")
